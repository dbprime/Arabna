...
/* ⚠️ REVERSED in V.07.3 — the owner's decision, and this is the ONE of the eight
   ...
   row's (294 against 328), and taking it out of both heads fixed that on
   its own. So the check inverts and keeps its teeth: NO head carries an
   arrow — and every head is still a real `<button>` that folds, so what
   was removed is a drawing and not a behaviour. */
ok('no group head carries an arrow any more', d.grpArrows === 0, d.grpArrows + ' arrows');
ok('…and every head is still a button that folds', await page.evaluate(() =>
  [...document.querySelectorAll('#drawer .dr-head')].length === 2 &&
  [...document.querySelectorAll('#drawer .dr-head')].every(h =>
    h.tagName === 'BUTTON' && h.hasAttribute('data-toggle') && h.hasAttribute('aria-expanded'))));
